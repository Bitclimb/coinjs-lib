...
class Transaction {
  ...
  hashForSignature (inIndex, prevOutScript, hashType) {
    ...
    const ourScript = bscript.compile(bscript.decompile(prevOutScript).filter(x => x !== opcodes.OP_CODESEPARATOR));
  ...
  hashForWitnessV0 (inIndex, prevOutScript, value, hashType) {
    ...
    if ((hashType & 0x1f) !== Transaction.SIGHASH_SINGLE &&
      (hashType & 0x1f) !== Transaction.SIGHASH_NONE) {
      const txOutsSize = this.outs.reduce((sum, output) => sum + 8 + varSliceSize(output.script), 0);
...
Transaction.fromHex = hex => Transaction.fromBuffer(Buffer.from(hex, 'hex'));
